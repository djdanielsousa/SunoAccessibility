...
from .publicLabels import getLabel
...
_ROLE_CHECKBOX = getattr(_ROLE_CONTAINER, "CHECKBOX", getattr(controlTypes, "ROLE_CHECKBOX", None))
...
def _nearbyNames(obj, limit=4):
	"""Collect nearby accessible names without walking far from the control."""
	names = []
	for direction in ("previous", "next"):
		current = obj
		for _unused in range(limit):
			current = _safeGet(current, direction)
			if not current:
				break
			name = _text(_safeGet(current, "name")).lower()
			if name:
				names.append(name)
	parent = _safeGet(obj, "parent")
	if parent:
		name = _text(_safeGet(parent, "name")).lower()
		if name:
			names.append(name)
	return names


def _publicLabel(key):
	"""Return the audited public-page label in NVDA's current language."""
	try:
		language = _text(languageHandler.getLanguage()).replace("-", "_")
	except Exception:
		language = "en"
...
def _countNear(obj):
	"""Find a short count exposed by Suno as a value or adjacent object."""
	attrs = _attrs(obj)
	for value in (_safeGet(obj, "value"), attrs.get("value"), attrs.get("aria-valuetext")):
		value = _text(value)
		if value and len(value) <= 20:
			return value
	for direction in ("next", "previous"):
		neighbor = _safeGet(obj, direction)
		value = _text(_safeGet(neighbor, "name"))
		if value and len(value) <= 20 and any(character.isdigit() for character in value):
			return value
	return u"—"
...
def _labelFor(obj):
	...
	if _role(obj, _ROLE_CHECKBOX):
		if any("instrumental" in name for name in _nearbyNames(obj)):
			return _publicLabel("instrumental")
	if _role(obj, _ROLE_BUTTON):
		nearbyNames = _nearbyNames(obj)
		# Icon-only controls in the advanced public creation panel.
		if tag == "button" and "hxc-btn-icon-only" in classes:
			if "hxc-btn-variant-primary" in classes and any(
				marker in name for name in nearbyNames for marker in ("auto", "write lyrics", "escrever letra")
			):
				return _publicLabel("back")
			if "hxc-btn-variant-frosted-glass" in classes:
				if any("style" in name or "estilo" in name for name in nearbyNames):
					return _publicLabel("styles")
				if any("audio" in name or "áudio" in name for name in nearbyNames):
					return _publicLabel("audio")
			if "hxc-btn-size-mini" in classes and any(
				"style" in name or "estilo" in name for name in nearbyNames
			):
				return _publicLabel("refreshStyles")
...
def _normalizedPublicName(obj):
	"""Improve Suno's generic names while retaining the exposed count."""
	name = _text(_safeGet(obj, "name")).lower()
	if name == "play button with play count":
		return _publicLabel("playCount").format(count=_countNear(obj))
	if name == "like button with like count":
		return _publicLabel("likeCount").format(count=_countNear(obj))
	return ""
...
class GlobalPlugin(globalPluginHandler.GlobalPlugin):
	...
	def event_NVDAObject_init(self, obj):
		try:
			if not _inSuno(obj):
				return
			normalizedName = _normalizedPublicName(obj)
			if normalizedName:
				obj.name = normalizedName
			elif not _text(getattr(obj, "name", "")):
				label = _labelFor(obj)
				if label:
					obj.name = label
